 ...
 * Generates crisp, minimalist UI and game sound effects
 */
...
class SoundEngine {
  ...
  private musicVolume: number = 0.6; // 0.0 to 1.0
  ...
  // Task intermediate step / confirmation blip
  public playTaskStep() {
    if (this.isMuted) return;
    this.initCtx();
    if (!this.ctx) return;
  ...
  // Sabotage trigger audio cue
  public playSabotageTrigger() {
    if (this.isMuted) return;
    this.initCtx();
    if (!this.ctx) return;
  ...
  // Victory / Defeat chord
  public playFanfare(victory: boolean) {
    if (this.isMuted) return;
    this.initCtx();
    if (!this.ctx) return;
    ...
    try {
      const now = this.ctx.currentTime;
      const notes = victory ? [523.25, 659.25, 783.99, 1046.5] : [440, 415.3, 392, 349.23];
      notes.forEach((freq, idx) => {
        const osc = this.ctx!.createOscillator();
        const gain = this.ctx!.createGain();
